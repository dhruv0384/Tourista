...
def get_all_activities_in_a_trip():
	return "select activity_date, activity_name, cost, activity_start_time, activity_end_time, activity_id, attraction_name from activity natural join trip where username = '" + session['username'] + "' and is_booked = false;";
...
# @app.route('/add-to-trip/<attraction_index>', methods=['GET', 'POST'])
...
# Receive attraction data to turn into an activity
@app.route('/add-to-trip', methods=['GET', 'POST'])
def add_to_trip():
